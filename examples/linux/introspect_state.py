...
from figurative.core.plugin import StateDescriptor
from figurative.utils.enums import StateStatus
from time import sleep
import typing
import argparse

parser = argparse.ArgumentParser(
    description="Explore a binary with Figurative and print the tree of states"
)
parser.add_argument(
    "binary",
    type=str,
...
def run_every(callee: typing.Callable, duration: int = 3) -> typing.Callable:
    ...
    def inner(thread):  # Takes `thread` as argument, which is provided by the daemon thread API
        while True:
            # Pass Figurative's state descriptor dict to the callee
            callee(thread.figurative.introspect())
            sleep(duration)

    return inner


m = Figurative(args.binary)

# Tell Figurative to run `print_fork_tree` every second
