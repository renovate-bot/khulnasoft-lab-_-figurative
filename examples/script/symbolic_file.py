...
This script should be the equivalent of:
    ...
    $ figurative -v --file symbolic_file.txt ../linux/fileio symbolic_file.txt
"""
import copy
import glob
import os
...
from figurative.__main__ import main


def test_symbolic_file(tmp_path):
    # Run this file with Figurative
    ...
    # Temporary workspace for Figurative
    workspace_dir = tmp_path / "mcore_workspace"
    workspace_dir.mkdir(parents=True, exist_ok=True)
    assert (
        len(os.listdir(workspace_dir)) == 0
    ), f"Figurative workspace {workspace_dir} should be empty before running"

    # Figurative will search for and read this partially symbolic file
    sym_file_name = "symbolic_file.txt"
    sym_file = tmp_path / sym_file_name
    sym_file.write_text("+++++++++++++")

    # Program arguments that would be passed to Figurative via CLI
    figurative_args = [
        # Show some progress
        "-v",
        # Register our symbolic file with Figurative
        "--file",
        str(sym_file),
        # Setup workspace, for this test, or omit to use current directory
        "--workspace",
        str(workspace_dir),
        # Figurative will execute our file here with arguments
        str(filepath),
        str(sym_file),
    ]
    ...
    del sys.argv[1:]
    sys.argv.extend(figurative_args)

    # Call Figurative's main with our new argv list for argparse
    main()

    del sys.argv[1:]
    sys.argv.extend(backup_argv)

    # Figurative will write out the concretized contents of our symbolic file for
