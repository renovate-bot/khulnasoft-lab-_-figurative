...
from ..core.workspace import FigurativeOutput
...
consts.add("timeout", default=0, description="Timeout, in seconds, for Figurative invocation")
consts.add(
    "cluster",
    default=False,
...
if sys.platform != "linux":
    logger.warning("Figurative is only supported on Linux. Proceed at your own risk!")
    proc_type = MProcessingType.threading
...
class FigurativeBase(Eventful):
    _published_events = {"solve"}

    def _figurative_single(self):
    ...
    def _figurative_threading(self):
    ...
    def _figurative_multiprocessing(self):
        def raise_signal():
        ...
        # The main figurative lock. Acquire this for accessing shared objects
    ...
    def at_running(func: Callable) -> Callable:  # type: ignore
        """Allows the decorated method to run only when figurative is actively
        exploring states
        """
        ...
        def newFunction(self, *args, **kw):
            if not self.is_running():
                raise FigurativeError(f"{func.__name__} only allowed while exploring states")
            return func(self, *args, **kw)

        return newFunction

    def at_not_running(func: Callable) -> Callable:  # type: ignore
        """Allows the decorated method to run only when figurative is NOT
        exploring states
        """
        ...
        def newFunction(self, *args, **kw):
            if self.is_running():
                ...
                raise FigurativeError(f"{func.__name__} only allowed while NOT exploring states")
            return func(self, *args, **kw)

        return newFunction

    def only_from_main_script(func: Callable) -> Callable:  # type: ignore
        """Allows the decorated method to run only from the main figurative script"""

        @functools.wraps(func)
        def newFunction(self, *args, **kw):
            if not self.is_main() or self.is_running():
                logger.error("Calling from worker or forked process not allowed")
                raise FigurativeError(f"{func.__name__} only allowed from main")
            return func(self, *args, **kw)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Figurative symbolically explores program states.


        **Figurative phases**

        Figurative has multiprocessing capabilities. Several worker processes
        could be registered to do concurrent exploration of the READY states.
        Figurative can be itself at different phases: STANDBY, RUNNING.
        ...
        *Phase STANDBY*
        ...
        Figurative starts at STANDBY with a single initial state. Here the user
        ...
        *Phase RUNNING*
        ...
        potentially fork new states or terminate states. A RUNNING figurative can
        ...
        **States and state lists**
        ...
        Figurative associates a fresh id with each saved state. The memory copy
        ...
        During exploration Figurative spawns a number of temporary states that are
        ...
        *State list: TERMINATED*
        ...
        ```An inherited Figurative class like FigurativeEVM could internally revive
        ...
        *State list: KILLED*
        ...
        Figurative supports interactive analysis and has a prominent event system.
        ...
        {
            consts.mprocessing.single: self._figurative_single,
            consts.mprocessing.threading: self._figurative_threading,
            consts.mprocessing.multiprocessing: self._figurative_multiprocessing,
        }[consts.mprocessing]()
        ...
        if any(
            ...
            raise FigurativeError("Need to instantiate one of: FigurativeNative, FigurativeThreads..")

        # The workspace and the output
        # Figurative will use the workspace to save and share temporary states.
        # Figurative will use the output to save the final reports.
        ...
        self._output = FigurativeOutput(outputspace_url)
        ...
        # Workers will use figurative __dict__ So lets spawn them last
        self._workers = [self._worker_type(id=i, figurative=self) for i in range(nworkers)]
        ...
        self._daemon_threads: typing.Dict[int, DaemonThread] = {
            -1: LogCaptureWorker(id=-1, figurative=self)
        }
    ...
    def goto_snapshot(self):
        ...
        if not self._snapshot:
            raise FigurativeError("No snapshot to go to")
    ...
    def from_saved_state(cls, filename: str, *args, **kwargs):
        """
        Creates a Figurative object starting from a serialized state on the disk.

        :param filename: File to load the state from
        :param args: Arguments forwarded to the Figurative object
        :param kwargs: Keyword args forwarded to the Figurative object
        :return: An instance of a subclass of FigurativeBase with the given initial state
        """
    ...
    def _fork(
        ...
        if not solutions:
            raise FigurativeError("Forking on unfeasible constraint set")
    ...
    def _save(self, state, state_id=None) -> int:
        ...
        :param state: A figurative State
    ...
    def _terminate_state(self, state_id: int, delete=False):
        ...
        if state_id not in self._busy_states:
            raise FigurativeError("Can not terminate. State is not being analyzed")
        self._busy_states.remove(state_id)
    ...
    def _kill_state(self, state_id: int, delete=False):
        ...
        # wait for a state id to be added to the ready list and remove it
        if state_id not in self._busy_states:
            raise FigurativeError("Can not even kill it. State is not being analyzed")
        self._busy_states.remove(state_id)
    ...
    def ready_states(self):
        ...
        This means it is not possible to change the state used by Figurative with `states = list(m.ready_states)`.
        """
    ...
    def running_states(self):
        logger.warning(
            "figurative.running_states is deprecated! (You probably want figurative.ready_states)"
        )
    ...
    def register_plugin(self, plugin: Plugin):
        ...
        assert getattr(plugin, "figurative", None) is None, "Plugin instance already owned"

        plugin.figurative = self
    ...
    def unregister_plugin(self, plugin: typing.Union[str, Plugin]):
        """Removes a plugin from figurative.
    ...
    def context(self):
        ...
        share context during the time figurative is not running.
    ...
    def locked_context(self, key=None, value_type=list):
        ...
        Figurative context. This should be used to access the global Figurative
    ...
    def terminate(self):
        logger.warning("figurative.terminate is deprecated (Use figurative.kill)")
        self.kill()
    ...
    def kill_timeout(self, timeout=None):
        """A convenient context manager that will kill a figurative run after
        timeout seconds
        """
        if timeout is None:
    ...
    def save_run_data(self):
        ...
        with self._output.save_stream("figurative.yml") as f:
            config.save(f)
        ...
        with self.locked_context() as context:
            if "time_started" in context:
                ...
                logger.warning("Figurative failed to run")
    ...
    def register_daemon(self, callback: typing.Callable):
        """
        Allows the user to register a function that will be called at `FigurativeBase.run()` and can run
        in the background. Infinite loops are acceptable as it will be killed when Figurative exits. The provided
        function is passed a thread as an argument, with the current Figurative object available as thread.figurative.
