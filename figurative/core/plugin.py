...
class Plugin(metaclass=DecorateAllMeta):
    __slots__ = ("figurative", "_enabled_key", "_plugin_context_name")
    ...
    def enable(self):
        ...
        with self.figurative.locked_context() as context:
    ...
    def locked_context(self, key=None, value_type=list):
        """
        A context manager that provides safe parallel access to the global Figurative context.
        This should be used to access the global Figurative context
        ...
        with self.figurative.locked_context(plugin_context_name, dict) as context:
            if key is None:
                yield context
            else:
    ...
    def context(self):
        ...
        plugin_context_name = self._plugin_context_name
        if plugin_context_name not in self.figurative.context:
            self.figurative.context[plugin_context_name] = {}
        return self.figurative.context[plugin_context_name]

    def on_register(self):
        """Called by parent figurative on registration"""
        pass

    def on_unregister(self):
        """Called be parent figurative on un-registration"""
        pass
...
class InstructionCounter(Plugin):
    def will_terminate_state_callback(self, state, ex):
        ...
        with self.figurative.locked_context() as figurative_context:
            figurative_instructions_count = figurative_context.get("instructions_count", 0)
            figurative_context["instructions_count"] = (
                figurative_instructions_count + state_instructions_count
            )
    ...
    def did_run_callback(self):
        _shared_context = self.figurative.context
...
class Visited(Plugin):
    ...
    def will_terminate_state_callback(self, state, ex):
        ...
        with self.figurative.locked_context() as figurative_context:
            figurative_visited = figurative_context.get("visited", set())
            figurative_context["visited"] = figurative_visited.union(state_visited)
    ...
    def did_run_callback(self):
        ...
        if self.coverage_file is not None:
            with self.figurative._output.save_stream(self.coverage_file) as f:
...
class Profiler(Plugin):
    ...
    def did_terminate_worker_callback(self, id):
        ...
        with self.figurative.locked_context("_profiling_stats", dict) as profiling_stats:
    ...
    def get_profiling_data(self):
        ...
        with self.figurative.locked_context("_profiling_stats") as profiling_stats:
            ps = None
...
class IntrospectionAPIPlugin(Plugin):
    ...
    def create_state(self, state_id: int):
        ...
        with self.locked_context("figurative_state", dict) as context:
    ...
    def will_run_callback(self, ready_states: typing.Generator):
        """
        Called at the beginning of FigurativeBase.run(). Creates a state descriptor for each of the ready states.
