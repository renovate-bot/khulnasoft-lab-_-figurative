...
consts.add(
    "fast_fail",
    False,
    "Kill Figurative if _any_ state encounters an unrecoverable exception/assertion.",
)
...
class Worker:
    ...
    will be spawned at Figurative constructor and will stay alive until killed.
    ...
    def __init__(self, *, id, figurative, single=False):
    ...
    def run(self, *args):
        ...
        logger.debug(
            "Starting Figurative Symbolic Emulator Worker %d. Pid %d Tid %d).",
            self.id,
            os.getpid(),
            threading.get_ident(),
        )
        ...
        with WithKeyboardInterruptAs(m.kill):
            ...
            # The worker runs until the figurative is killed
            while not m._killed.value:
                ...
                try:  # handle fatal errors even exceptions in the exception handlers
                    try:  # handle Concretize and TerminateState
                        ...
                        # that figurative we will let us stay at this phase for a _while_
                        ...
                        # Raises an Exception if figurative gets cancelled
                        ...
                        # Allows to terminate figurative worker on user request
                    ...
                    if current_state is not None:
                        ...
                        current_state = None
                    if consts.fast_fail:
                        # Kill Figurative if _any_ state encounters unrecoverable
                        # exception/assertion
                        m.kill()
                    break
...
class DaemonThread(WorkerThread):
    """
    Special case of WorkerThread that will exit whenever the main Figurative process exits.
    """
...
def render_state_descriptors(desc: typing.Dict[int, StateDescriptor]):
    ...
    :param desc: Output from FigurativeBase.introspect
