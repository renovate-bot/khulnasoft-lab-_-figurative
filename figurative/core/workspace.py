...
class Testcase:
    """
    A `WorkspaceTestCase` represents a test case input generated by Figurative.
    """

    def __init__(self, workspace: "FigurativeOutput", prefix: str):
...
class Store:
    ...
    def fromdescriptor(cls, desc):
        """
        Create a :class:`~figurative.core.workspace.Store` instance depending on the descriptor.
    ...
    def save_state(self, state: StateBase, key: str) -> None:
        ...
        :param figurative.core.StateBase state:
...
class FilesystemStore(Store):
    """
    A directory-backed Figurative workspace
    """
...
class MemoryStore(Store):
    """
    An in-memory (dict) Figurative workspace.
    ...
    def stream(self, key, mode="r", lock=False):
        if lock:
            raise FigurativeError("mem: does not support concurrency")
        if "b" in mode:
...
class RedisStore(Store):
    """
    A redis-backed Figurative workspace
    """
...
class FigurativeOutput:
    """
    Functionality related to producing output. Responsible for generating state summaries,
    coverage information, etc.

    Invoked only from :class:`figurative.Figurative` from a single parent process, so
    locking is not required.
    """

    def __init__(self, desc=None):
        """
        Create an object capable of producing Figurative output.
