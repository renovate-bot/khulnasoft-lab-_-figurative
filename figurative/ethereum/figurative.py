...
from ..core.figurative import FigurativeBase, FigurativeError
...
class FigurativeEVM(FigurativeBase):
    """Figurative EVM manager

    Usage Ex::

        from figurative.ethereum import FigurativeEVM, ABI
        m = FigurativeEVM()
    ...
    def make_symbolic_buffer(self, size, name=None, avoid_collisions=False):
        ...
        You can operate on it normally and add constraints to figurative.constraints
        via figurative.constrain(constraint_expression)
            ...
            while pos < len(hex_contract):
                if hex_contract[pos] == "_":
                    ...
                    if ":" in lib_placeholder:
                        # __/tmp/tmp_9k7_l:Figurative______________
    ...
    def _compile(source_code, contract_name, libraries=None, crytic_compile_args=None):
        ...
        if (
            ...
            with tempfile.NamedTemporaryFile("w+", suffix=".sol") as temp:
                ...
                temp.flush()
                compilation_result = FigurativeEVM._compile_through_crytic_compile(
    ...
    def __init__(self, plugins=None, **kwargs):
        """
        A Figurative EVM manager
        :param plugins: the plugins to register in this figurative manager
        """
    ...
    def create_contract(self, owner, balance=0, address=None, init=None, name=None, gas=None):
        ...
        if self.count_ready_states():
            self._accounts[name] = EVMContract(
                address=address, figurative=self, default_caller=owner, name=name
            )
    ...
    def create_account(self, balance=0, address=None, code=None, name=None, nonce=None):
        ...
        self._accounts[name] = EVMAccount(address, figurative=self, name=name)
        return self.accounts[name]
    ...
    def _migrate_tx_expressions(self, state, caller, address, value, data, gas, price):
        ...
        # global symbolic variables via FigurativeEVM.make_.... and those
        ...
        for state in self.ready_states:
            ...
            if address is None:
                if issymbolic(caller):
                    ...
                    raise EthereumError(
                        "Figurative does not currently support contracts with symbolic addresses creating new contracts"
                    )
    ...
    def run(self, **kwargs):
        ...
        # reverted (or not very interesting) FigurativeEVM uses another list:
        # saved_states
        ...
        # FigurativeEthereum decided at terminate_state_callback which state is
    ...
    def _terminate_state_callback(self, state, e):
        ...
        if tx.is_human:
            ...
            logger.info(
                "Figurative exception: state should be terminated only at the end of the human transaction"
            )
        """
    ...
    def _generate_testcase_ex(self, state, message="", name="user"):
        ...
        Registered plugins and other parts of Figurative add files to the Testcase.
        ...
        :param figurative.core.state.State state:
        ...
        if not self.fix_unsound_symbolication(state):
            raise FigurativeError("Trying to generate a testcase out of an unsound state path")
