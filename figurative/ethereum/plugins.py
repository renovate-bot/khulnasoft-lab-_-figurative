...
class FilterFunctions(Plugin):
    ...
    def will_open_transaction_callback(self, state, tx):
        ...
        if state.context.get("constrained%d" % id(self), 0) != tx_cnt:
            ...
            md = self.figurative.get_metadata(tx.address)
            if md is None:
                return
...
class LoopDepthLimiter(Plugin):
    ...
    def will_run_callback(self, *args):
        with self.figurative.locked_context("seen_rep", dict) as reps:
            reps.clear()
...
class VerboseTraceStdout(Plugin):
    """
    Same as VerboseTrace but prints to stdout. Note that you should use it only if Figurative
    ...
    def did_run_callback(self):
        ...
        with self.figurative.locked_context("ethereum.saved_states", list) as saved_states:
            ...
            for state_id in list(saved_states):
                st = self.figurative._load(state_id)
                if not st.context["written"][-1]:
                    if st.id in self.figurative._ready_states:
                        self._publish(
                        ...
                        self.figurative._ready_states.remove(st.id)
                        self.figurative._terminated_states.append(st.id)
                        self._publish(
