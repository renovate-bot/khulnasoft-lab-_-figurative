"""
$python figurative-verifier.py property.sol TestToken
"""
import os
import re
import sys
...
from figurative.ethereum.detectors import DetectIntegerOverflow
from figurative.ethereum.plugins import FilterFunctions, VerboseTrace, KeepOnlyIfStorageChanges
from figurative.core.smtlib.operators import OR, NOT, AND
from figurative.ethereum.abi import ABI
from figurative.utils.log import set_verbosity
from prettytable import PrettyTable
from figurative.utils import config
from figurative.utils.nointerrupt import WithKeyboardInterruptAs


def constrain_to_known_func_ids(state):
    world = state.platform
    tx = world.human_transactions[-1]
    md = state.figurative.get_metadata(tx.address)
...
def figurative_verifier(
    ...
    The results are dumped to stdout and to the workspace folder.
        ...
        $figurative-verifier property.sol  --contract TestToken --smt.solver yices --maxt 4
    ...
    print("# Welcome to figurative-verifier")
    # Main figurative manager object
    m = FigurativeEVM()
...
def main():
    ...
    current_version = pkg_resources.get_distribution("figurative").version
    parser.add_argument(
        "--version",
        action="version",
        version=f"Figurative {current_version}",
    ...
    eth_flags.add_argument(
        "--thorough-mode",
        action="store_true",
        help="Configure Figurative for more exhaustive exploration. Evaluate gas, generate testcases for dead states, "
    ...
    logger = logging.getLogger("figurative.main")
