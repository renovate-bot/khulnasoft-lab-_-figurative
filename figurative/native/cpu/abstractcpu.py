...
class InstructionEmulationError(CpuException):
    """
    Exception raised when failing to emulate an instruction outside of Figurative.
    """
...
class Operand:
    ...
    def _reg_name(self, reg_id: int):
        ...
        register name based on figurative's alias in the register file
...
class Abi:
    ...
    def invoke(self, model, prefix_args=None):
        ...
        :func:`~figurative.models.isvariadic` returns true for `model`, `model` receives a single
...
platform_logger = logging.getLogger("figurative.platforms.platform")
...
class Cpu(Eventful):
    ...
    used by platforms and py:class:figurative.core.Executor
    ...
    def emulate_until(self, target: int):
        ...
        :param target: Where Unicorn should hand control back to Figurative. Set to 0 for all instructions.
        """
        self._concrete = True
        self._break_unicorn_at = target
        if self.emu:
            self.emu.write_backs_disabled = False
            self.emu.load_state_from_figurative()
    ...
    def _wrap_operands(self, operands):
        ...
        See :class:`~figurative.core.cpu.abstractcpu.Operand` class
        """
        raise NotImplementedError
    ...
    def execute(self):
        ...
        try:
            if self._concrete and "SYSCALL" in name:
                self.emu.sync_unicorn_to_figurative()
            if self._concrete and "SYSCALL" not in name:
                self.emulate(insn)
                if self.PC == self._break_unicorn_at:
                    logger.debug("Switching from Unicorn to Figurative")
