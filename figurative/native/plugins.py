...
class Merger(Plugin):
    def on_register(self):
        p = self.figurative._publish
        self.figurative._publish = lambda *args, **kwargs: None
        for state in self.figurative.ready_states:
            self.did_fork_state_callback(state, None, None, None, None)

        self.figurative._publish = p

    def load_state(self, state_id, delete=False):
        """
        Loads a state in Figurative with state-id = `state_id` by using the corresponding API in Executor
        ...
        :return: None
        """
        state = self.figurative._workspace.load_state(state_id, delete=delete)
        return state

    def delete_state(self, state_id):
        """
        Deletes a state in Figurative with state-id = `state_id` by using the corresponding API in Executor
        :param state_id: state-id for state that is being deleted
        :return: None
        """
        self.figurative.kill_state(state_id, delete=True)

    def replace_state(self, state_id, state):
        """
        Replaces a state in Figurative with state-id = `state_id` by using the corresponding API in Executor
        ...
        :return: None
        """
        return self.figurative._save(state, state_id=state_id)
