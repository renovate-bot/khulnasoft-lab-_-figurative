...
class Transaction:
    ...
    def dump(self, stream, state, mevm, conc_tx=None):
        ...
        :param figurative.ethereum.State state: state that the tx exists in
        :param figurative.ethereum.FigurativeEVM mevm: figurative instance
        :return:
        """
        from ..ethereum import ABI  # circular imports
        from ..ethereum.figurative import flagged
...
class EVMWorld(Platform):
    ...
    def calculate_new_address(sender=None, nonce=None):
        if sender is None:
            ...
            raise EthereumError(
                "Figurative does not yet support contracts with symbolic addresses creating new contracts"
            )
        else:
            if nonce is None:
    ...
    def dump(self, stream, state, mevm, message):
        from ..ethereum.figurative import calculate_coverage, flagged
