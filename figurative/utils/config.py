...
The config values and constant are gathered from three sources:
    ...
    2. yml files (i.e. ./figurative.yml)
...
def parse_config(f):
    ...
    # to reading poorly formatted values
        ...
        raise ConfigError("Failed reading config file. Do you have a local [.]figurative.yml file?")
...
def load_overrides(path=None):
    ...
    Default paths: ./mcore.yml, ./.mcore.yml, ./figurative.yml, ./.figurative.yml.
    """

    if path is not None:
        names = [path]
    else:
        possible_names = ["mcore.yml", "figurative.yml"]
