...
def convert_permissions(m_perms: str):
    """
    Converts a Figurative permission string into a Unicorn permission
    :param m_perms: Figurative perm string ('rwx')
...
class ConcreteUnicornEmulator:
    ...
    implementations in Figurative. This Emulator is instead intended to completely replace Figurative's executor when
    ...
    address at which it should switch back from Unicorn to Figurative. Passing 0 will result in the entire target being
    executed concretely.
    ...
    the point where symbolic data is introduced, switch to Figurative, fork states, then switch back, it *definitely*
    won't work.
    ...
    def __init__(self, cpu):
        ...
        # Registers to ignore when translating figurative context to unicorn
        ...
        self.load_state_from_figurative()
    ...
    def load_state_from_figurative(self) -> None:
    ...
    def map_memory_callback(
        ...
        """
        Catches did_map_memory and copies the mapping into Figurative
        """
        begin = address
        end = address + size
    ...
    def unmap_memory_callback(self, start, size):
        """Unmap Unicorn maps when Figurative unmaps them"""
    ...
    def _hook_syscall(self, uc, data):
        """
        Unicorn hook that transfers control to Figurative so it can execute the syscall
        """
        logger.debug(
            f"Stopping emulation at {uc.reg_read(self._to_unicorn_id('RIP')):#x} to perform syscall"
        )
        self.sync_unicorn_to_figurative()
    ...
    def _hook_cpuid(self, uc, data):
        """
        Unicorn hook that uses Figurative's semantics for cpuid
        """
    ...
    def emulate(self, instruction) -> None:
        ...
        # or bring a value from Figurative state.
        while True:
    ...
    def _step(self, instruction, chunksize: int = 0) -> None:
        ...
        if self._cpu.PC == self._stop_at:
            logger.info("Reached emulation target, switching to Figurative mode")
            self.sync_unicorn_to_figurative()
            self._stop_at = None
    ...
    def sync_unicorn_to_figurative(self):
        """
        Copy registers and written memory back into Figurative
        """
        self.write_backs_disabled = True
        for reg in self.registers:
            # Ignore registers that aren't supported by unicorn
        ...
        if len(self._mem_delta) > 0:
            logger.debug(f"Syncing {len(self._mem_delta)} writes back into Figurative")
    ...
    def write_back_memory(self, where, expr, size):
        """Copy memory writes from Figurative back into Unicorn in real-time"""
    ...
    def write_back_register(self, reg, val):
        """Sync register state from Figurative -> Unicorn"""
        # Ignore registers that aren't supported by unicorn
        if reg in self.ignore_registers:
            return
        if self.write_backs_disabled:
