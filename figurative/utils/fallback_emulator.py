...
class UnicornEmulator:
    ...
    def _hook_xfer_mem(self, uc, access, address, size, value, data):
        ...
        # in from Figurative state. If we try to mem_write it here, Unicorn
    ...
    def emulate(self, instruction, reset=True):
        ...
        while True:
            ...
            if reset:
                ...
                # Establish Figurative state, potentially from past emulation
                # attempts
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            for register in registers:
                logger.debug(
                    ...
                    f"Figurative: {self._cpu.read_register(register):08x}, "
                    f"Unicorn {self._emu.reg_read(self._to_unicorn_id(register)):08x}"
                )
            logger.debug(">" * 10)

        # Bring back Unicorn registers to Figurative
        for reg in registers:
