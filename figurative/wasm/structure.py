...
consts.add(
    "decode_names",
    default=False,
    description="Should Figurative attempt to decode custom name sections",
)
...
class Module:
    ...
    def load(cls, filename: str):
        """
        Converts a WASM module in binary format into Python types that Figurative can understand
...
class ModuleInstance(Eventful):
    ...
    def invoke(self, stack: "Stack", funcaddr: FuncAddr, store: Store, argv: typing.List[Value]):
        ...
        # caller. Since Figurative executes functions via the `run` method (ie separately from invocation), we don't
        # do that here.
...
class ConcretizeCondition(Concretize):
    """Tells Figurative to concretize a condition required to direct execution."""
