...
class TraceVisualizer:
    ...
    def visualize(self):
        """
        Given a Figurative workspace, or trace file, highlight the basic blocks.
        """
    ...
    def set_comment_at_xref(self, xref, comment):
        try:
            ...
            w = "FigurativeTrace: Could not lookup " + hex(xref.address)
...
def get_workspace():
    choice = get_choice_input("Select Trace Type", "Input", ["Trace File", "Figurative Workspace"])
    if choice == 0:
...
def viz_trace(view):
    """
    Given a Figurative trace file, highlight the basic blocks.
    """
...
PluginCommand.register(
    "FigurativeTrace: Highlight", "Highlight Figurative Execution Trace", viz_trace
)
PluginCommand.register(
    "FigurativeTrace: BB Coverage", "Compute cumulative BB coverage for each function ", get_coverage
)
PluginCommand.register(
    "FigurativeTrace: Live Highlight",
    "Highlight Figurative Execution Trace at Real-Time",
    viz_live_trace,
)
PluginCommand.register("FigurativeTrace: Clear", "Clear Figurative Trace Highlight", clear_all)
