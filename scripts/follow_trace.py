...
from figurative.core.plugin import ExtendedTracer, Plugin
...
def main():
    ...
    # Create a concrete Figurative and record it
    m1 = Figurative.linux(args.cmd[0], args.cmd[1:])
    t = ExtendedTracer()
    r = TraceReceiver(t)
    ...
    # Create a symbolic Figurative and follow last trace
    symbolic_args = ["+" * len(arg) for arg in args.cmd[1:]]
    m2 = Figurative.linux(args.cmd[0], symbolic_args)
    f = Follower(r.trace)
    if range:
