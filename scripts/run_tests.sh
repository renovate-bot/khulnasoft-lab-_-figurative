...
run_truffle_tests(){
    ...
    coverage run -m figurative . --contract MetaCoin --workspace output --exclude-all --thorough-mode --evm.oog ignore --evm.txfail optimistic --smt.solver portfolio
    # Truffle smoke test. We test if figurative is able to generate states
...
run_tests_from_dir() {
    ...
    pytest --durations=100 --cov=figurative --cov-config=$GITHUB_WORKSPACE/.coveragerc -n auto "tests/$DIR"
    RESULT=$?
    return $RESULT
}
