...
sandshrew.py
    ...
    utilizing Figurative as a backend for symbolic execution and Unicorn
...
from figurative.core.smtlib import operators
...
def binary_symbols(binary):
    ...
    We do this in order to provide the symbols Figurative should hook on to
...
def main():
    ...
    # initialize Figurative
    m = Figurative.linux(args.test, ["+" * BUFFER_SIZE])
    m.verbosity(2)
    ...
    for sym in m.context["syms"]:
        ...
        def concolic_hook(state):
            ...
            with m.locked_context() as context:
                ...
                logging.debug("Continuing with Figurative symbolic execution")

    # TODO(alan): resolve ifunc (different archs use different optimized implementations)
    @m.hook(m.resolve(args.cmp_sym))
    def cmp_model(state):
        """
        used in order to invoke Figurative function model for strcmp and/or other comparison operation
