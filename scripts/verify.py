...
from figurative.platforms import linux_syscalls
...
## figurative context would be heavier than needed.
...
def cmp_regs(cpu, should_print=False):
    ...
    :param figurative.core.cpu Cpu: Current cpu
...
def post_mcore(state, last_instruction):
    ...
    # Synchronize qemu state to figurative's after a system call
    ...
    # If we executed a few instructions of a helper, we need to sync Figurative's
...
def sync_svc(state):
    """
    Mirror some service calls in figurative. Happens after qemu executed a SVC
    instruction, but before figurative did.
    """
    syscall = state.cpu.R7  # Grab idx from figurative since qemu could have exited
...
def initialize(state):
    """
    Synchronize the stack and register state (figurative->qemu)
    """
