...
from figurative.core.plugin import (
    InstructionCounter,
...
from figurative.core.state import StateBase, TerminateState
from figurative.core.worker import WorkerProcess, WorkerThread
from figurative.ethereum import FigurativeEVM
from figurative.native import Figurative
from figurative.utils.enums import StateLists, StateStatus
from figurative.utils.helpers import deque
from figurative.utils.log import CallbackStream, FigurativeContextFilter

from .evm_utils import setup_detectors_flags
from .introspect_plugin import FigurativeServerIntrospectionPlugin
from .FigurativeServer_pb2 import *
from .FigurativeServer_pb2_grpc import (
    FigurativeServerServicer,
    add_FigurativeServerServicer_to_server,
)
...
class FigurativeWrapper:
    def __init__(
        self, mcore_object: Figurative, thread_target: Callable, *thread_args: Any
    ):
        self.uuid: str = uuid.uuid4().hex
        self.figurative_object: Figurative = mcore_object
        # mimics Figurative repository, reasoning for the queue size difference is provided in Figurative:
        # https://github.com/khulnasoft-lab/figurative/blob/5a258f499098394c0af25e2e3f00b1b603c2334d/figurative/core/figurative.py#L133-L135
        self.log_queue = (
    ...
    def append_log(self, msg: str) -> None:
        ...
        try:
            ...
            # mimics Figurative repository, reasoning for using append and catching an AttributeError is provided in Figurative:
            # https://github.com/khulnasoft-lab/figurative/blob/5a258f499098394c0af25e2e3f00b1b603c2334d/figurative/core/worker.py#L297-L303
            if q.full():
                q.get()
            q.put(msg)


class FigurativeServicer(FigurativeServerServicer):
    """Provides functionality for the methods set out in the protobuf spec"""

    def __init__(self, stop_event: Event):
        """Initializes the dict that keeps track of all created figurative instances, as well as avoid/find address set"""

        self.figurative_instances: Dict[str, FigurativeWrapper] = {}
        self.stop_event: Event = stop_event

        figurative_logger = logging.getLogger("figurative")
        figurative_logger.parent = None
        figurative_logger.setLevel(logging.WARNING)
        ...
        custom_log_handler.addFilter(FigurativeContextFilter())

        figurative_logger.addHandler(custom_log_handler)

    def log_callback(self, msg: str):
        logger.debug(msg)
        thread_name, msg_content = msg.split(" ", 1)

        if thread_name in self.figurative_instances:
            # This will always be True if multiprocessing or single is used, since all WorkerProcess/WorkerSingle
            # instances will share the same Thread name as the FigurativeWrapper's mthread which is added on Start
            self.figurative_instances[thread_name].append_log(msg_content)
        else:
            for mwrapper in filter(
                lambda x: x.figurative_object._worker_type == WorkerThread,
                list(self.figurative_instances.values())[::-1],
            ):  # Thread name/idents can be reused, so start search from most recently-started instance
                for worker in mwrapper.figurative_object._workers + list(
                    mwrapper.figurative_object._daemon_threads.values()
                ):
    ...
    def StartNative(
        ...
        """Starts a singular Figurative instance to analyze a native binary"""
        try:
        ...
        try:
            m = Figurative.linux(
                ...
                introspection_plugin_type=FigurativeServerIntrospectionPlugin,
            )
        except Exception as e:
            logger.warning(e)
            context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
        ...
        try:
            ...
            def figurative_native_runner(mcore_wrapper: FigurativeWrapper):
                mcore_wrapper.figurative_object.run()
                mcore_wrapper.final_states = {
                    k: dataclasses.replace(v)
                    for k, v in mcore_wrapper.figurative_object.introspect().items()
                }
                mcore_wrapper.figurative_object.finalize()

            figurative_wrapper = FigurativeWrapper(m, figurative_native_runner)
            self.figurative_instances[figurative_wrapper.uuid] = figurative_wrapper

            def state_callback_hook(state: StateBase):
                callbacks = figurative_wrapper.state_callbacks[state.id]
                for callback in callbacks:
                    callback(state)

            m.hook(None)(state_callback_hook)

            figurative_wrapper.start()

        except Exception as e:
            logger.warning(e)
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(
                "Figurative failed to start or crashed during execution!"
            )
            return FigurativeInstance()

        return FigurativeInstance(uuid=figurative_wrapper.uuid)

    def StartEVM(
        self, evm_arguments: EVMArguments, context: _Context
    ) -> FigurativeInstance:
        """Starts a singular Figurative instance to analyze a solidity contract"""
        ...
        try:
            ...
            def figurative_evm_runner(
                mcore_wrapper: FigurativeWrapper, args: argparse.Namespace
            ):

                mcore_wrapper.figurative_object.multi_tx_analysis(
                ...
                if not args.no_testcases:
                    ...
                    mcore_wrapper.figurative_object.kill()

            figurative_wrapper = FigurativeWrapper(m, figurative_evm_runner, args)
            self.figurative_instances[figurative_wrapper.uuid] = figurative_wrapper
            figurative_wrapper.start()
    ...
    def Terminate(
        self, mcore_instance: FigurativeInstance, context: _Context
    ) -> TerminateResponse:
        """Terminates the specified Figurative instance."""
        if mcore_instance.uuid not in self.figurative_instances:
            context.set_code(grpc.StatusCode.FAILED_PRECONDITION)
            context.set_details("Specified Figurative instance not found!")
            return TerminateResponse()

        m_wrapper = self.figurative_instances[mcore_instance.uuid]

        if not (
            m_wrapper.figurative_object.is_running() and m_wrapper.thread.is_alive()
        ):
            return TerminateResponse()
        m_wrapper.figurative_object.kill()
        return TerminateResponse()

    def GetStateList(
        self, mcore_instance: FigurativeInstance, context: _Context
    ) -> FigurativeStateList:
        """Returns full list of states for given FigurativeInstance."""
        active_states = []
        waiting_states = []
        paused_states = []
        forked_states = []
        ...
        if mcore_instance.uuid not in self.figurative_instances:
            context.set_code(grpc.StatusCode.FAILED_PRECONDITION)
            context.set_details("Specified Figurative instance not found!")
            return FigurativeStateList()

        mcore_wrapper = self.figurative_instances[mcore_instance.uuid]
        states = (
            mcore_wrapper.final_states
            if mcore_wrapper.final_states is not None
            else mcore_wrapper.figurative_object.introspect()
        )
    ...
    def GetMessageList(
        self, mcore_instance: FigurativeInstance, context: _Context
    ) -> FigurativeMessageList:
        """Returns any new log messages for given FigurativeInstance since the previous call.
        Currently, implementation is based on TUI."""
        if mcore_instance.uuid not in self.figurative_instances:
            context.set_code(grpc.StatusCode.FAILED_PRECONDITION)
            context.set_details("Specified Figurative instance not found!")
            return FigurativeMessageList()

        q = self.figurative_instances[mcore_instance.uuid].log_queue
        i = 0
        messages = []
        while not q.empty():
            msg = FigurativeLogMessage(content=q.get())
            messages.append(msg)
            i += 1
        return FigurativeMessageList(messages=messages)

    def CheckFigurativeRunning(
        self, mcore_instance: FigurativeInstance, context: _Context
    ) -> FigurativeRunningStatus:

        if mcore_instance.uuid not in self.figurative_instances:
            context.set_code(grpc.StatusCode.FAILED_PRECONDITION)
            context.set_details("Specified Figurative instance not found!")
            return FigurativeRunningStatus()

        m_wrapper = self.figurative_instances[mcore_instance.uuid]

        return FigurativeRunningStatus(is_running=(m_wrapper.thread.is_alive()))

    def StopServer(
        self, request: StopServerRequest, context: _Context
    ) -> StopServerResponse:
        to_warn = False
        for mwrapper in self.figurative_instances.values():
        ...
        if to_warn:
            warning_message = "WARNING: Not all Figurative processes were shut down successfully before timeout. There may be extra processes running even after the server has stopped."
            context.set_code(grpc.StatusCode.INTERNAL)
    ...
    def ControlState(
        ...
        if request.figurative_instance.uuid not in self.figurative_instances:
            context.set_code(grpc.StatusCode.FAILED_PRECONDITION)
            context.set_details("Specified Figurative instance not found!")
            return ControlStateResponse()

        mcore_wrapper = self.figurative_instances[request.figurative_instance.uuid]
        mcore_object = mcore_wrapper.figurative_object
        states = (
            mcore_wrapper.final_states
            if mcore_wrapper.final_states is not None
        ...
        # Pause / Resume / Kill should have no effect if Figurative is no longer running,
...
def main():
    ...
    add_FigurativeServerServicer_to_server(FigurativeServicer(stop_event), server)
