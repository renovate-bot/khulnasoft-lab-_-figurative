...
library Figurative{
    ...
    function terminate(string message){
        /*
         Terminate global figurative search with an optional termination message
        */
        assembly {
            jump(0x4141414141414141414141414141414141414141)
        }
...
contract Reachable {
    ...
    function entry(uint input) {
        ...
        Figurative.print("Initializing!");
        if (Figurative.is_symbolic(storage_variable))
            Figurative.print("No! storage_variable should not be symbolic");

        //this.write_then_throw();
        this.call.gas(10001).value(msg.value)(bytes4(sha3("function write_then_throw()")));

        if (Figurative.is_symbolic(storage_variable))
            Figurative.print("Now It could be symbolic..");

        if (Figurative.can_be_true(storage_variable == 0x585858)){
            Figurative.terminate("Goal Found!");
        }
