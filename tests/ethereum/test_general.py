...
from figurative import FigurativeError
from figurative.core.plugin import Plugin
from figurative.core.smtlib import ConstraintSet, operators, PortfolioSolver, SolverType
from figurative.core.smtlib.expression import BitVec, BitVecVariable
from figurative.core.smtlib.visitors import to_constant
from figurative.core.state import TerminateState
from figurative.ethereum import (
    FigurativeEVM,
    State,
...
from figurative.ethereum.plugins import FilterFunctions
from figurative.ethereum.solidity import SolidityMetadata
from figurative.platforms import evm
from figurative.platforms.evm import EVMWorld, ConcretizeArgument, concretized_args, Return, Stop
from figurative.utils.deprecated import FigurativeDeprecationWarning
...
def disposable_mevm(*args, **kwargs):
    mevm = FigurativeEVM(*args, **kwargs)
    try:
        yield mevm
    finally:
...
class EthVerifierIntegrationTest(unittest.TestCase):
    def test_propverif(self):
        ...
        with smtcfg.temp_vals():
            ...
            with contextlib.redirect_stdout(f):
                verifier.figurative_verifier(filename, "TestToken")
            output = f.getvalue()
    ...
    def test_propverif_external(self) -> None:
        cli_version = subprocess.check_output(("figurative-verifier", "--version")).decode("utf-8")
        cli_version = cli_version.split(
            "Figurative is only supported on Linux. Proceed at your own risk!\n"
        )[-1]
        py_version = f"Figurative {pkg_resources.get_distribution('figurative').version}\n"
...
class EthTests(unittest.TestCase):
    ...
    def test_check_jumpdest_symbolic_pc(self):
        """
        In Figurative 0.2.4 (up to 6804661) when run with DetectIntegerOverflow,
    ...
    def test_call_with_concretized_args(self):
        ...
        https://github.com/khulnasoft-lab/figurative/issues/1237
        """
        m = self.mevm
    ...
    def test_event_forwarding_after_state_fork_during_message_call(self):
        # https://github.com/khulnasoft-lab/figurative/issues/1255
        source_code = """
...
class EthSolidityMetadataTests(unittest.TestCase):
    ...
    def test_abi_constructor_and_fallback_items(self):
        with disposable_mevm() as m:
            ...
            with self.assertWarns(FigurativeDeprecationWarning):
...
class EthSpecificTxIntructionTests(unittest.TestCase):
    def test_jmpdest_check(self):
        ...
        https://github.com/khulnasoft-lab/figurative/issues/1169
        """
...
class EthPluginTests(unittest.TestCase):
    ...
    def test_checkpoint(self):
        ...
        self.assertRaises(FigurativeError, m.goto_snapshot)
        self.assertEqual(m.count_ready_states(), 1)
        # take the snap
        m.take_snapshot()
        self.assertEqual(m.count_ready_states(), 1)
    ...
    def test_is_main(self):
        ...
        class X(Plugin):
            def will_evm_execute_instruction_callback(self, state, instruction, args):
                is_main = self.figurative.is_main()
                is_running = self.figurative.is_running()
                with self.locked_context() as ctx:
