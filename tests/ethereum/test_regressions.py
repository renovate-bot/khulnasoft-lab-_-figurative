...
# TLDR: when we launch `python -m figurative` and one uses PyCharm remote interpreter
...
class IntegrationTest(unittest.TestCase):
    ...
    def _simple_cli_run(
        ...
        """
        Simply run the Figurative command line with `filename`
        ...
        command = [PYTHON_BIN, "-m", "figurative"]
    ...
    def test_solidity_timeout(self):
        ...
        output = list(
            filter(
                lambda l: b"Figurative is only supported on Linux. Proceed at your own risk!"
                not in l,
                output.splitlines(),
            )
        )
        ...
        # self.assertIn(b'm.c.figurative:INFO: Verbosity set to 1.', output[0])
        self.assertIn(b"m.main:INFO: Registered plugins: ", output[0])
        self.assertIn(b"m.main:INFO: Beginning analysis", output[1])
        self.assertTrue(
            any(b"m.e.figurative:INFO: Starting symbolic create contract" in o for o in output)
        )

        # self.assertIn(b'm.c.figurative:INFO: Results in ', output[-2])
        # self.assertIn(b'm.c.figurative:INFO: Total time: ', output[-1])

        # Since we count the total time of Python process that runs Figurative, it takes a bit more time
    ...
    def test_regressions_imports(self):
        """
        This tests Figurative on a contract that imports code from another directory.
        """
        issue = {
            "number": "main/main",
            "contract": "C",
    ...
    def test_addmod(self):
        ...
        from figurative.core.smtlib import ConstraintSet, Z3Solver, Operators
