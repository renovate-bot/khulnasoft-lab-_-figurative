...
from figurative.binary import Elf, CGCElf
from figurative.native.mappings import mmap, munmap
...
class NativeIntegrationTest(unittest.TestCase):
    ...
    def test_logger_verbosity(self) -> None:
        ...
        output = subprocess.check_output(
            [PYTHON_BIN, "-m", "figurative", "--no-color", "--workspace", workspace, filename]
        )
    ...
    def _test_arguments_assertions_aux(
        ...
        self.assertIn(b"m.n.figurative:INFO: Loading program", output[0])
        ...
        for i in range(testcases_number):
            ...
            expected1 = b"m.c.figurative:INFO: Generated testcase No. "
            # expected2 = b'- Program finished with exit status: '

            self.assertIn(expected1, line)
            # self.assertIn(expected2, line)

        # self.assertIn(b'm.c.figurative:INFO: Results in /tmp', output[2+testcases_number])
        # self.assertIn(b'm.c.figurative:INFO: Total time: ', output[2+testcases_number+1])
    ...
    def test_basic_arm(self) -> None:
        ...
        workspace = os.path.join(self.test_dir, "workspace")
        cmd = [PYTHON_BIN, "-m", "figurative", "--no-color", "--workspace", workspace, filename]
        ...
        # self.assertIn(b'm.c.figurative:INFO: Verbosity set to 1.', output[0])
        self.assertIn(b"m.n.figurative:INFO: Loading program ", output[0])
        self.assertIn(b"m.c.figurative:INFO: Generated testcase No. 0 - ", output[1])
        self.assertIn(b"m.c.figurative:INFO: Generated testcase No. 1 - ", output[2])
    ...
    def _test_no_crash(self, test_name: str, *args) -> None:
        """
        Tests that the specified test binary doesn't cause Figurative to crash.
        """
