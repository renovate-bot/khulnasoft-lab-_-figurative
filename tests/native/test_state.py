...
from figurative.core.state import StateBase
from figurative.utils.event import Eventful
from figurative.platforms import linux
from figurative.native.state import State
from figurative.core.smtlib import BitVecVariable, ConstraintSet
from figurative.native import Figurative
from figurative.native.plugins import Merger
from figurative.core.plugin import Plugin
from figurative.utils import config
from figurative.utils.helpers import pickle_dumps
...
class StateHooks(unittest.TestCase):
    def setUp(self):
        ...
        dirname = os.path.dirname(__file__)
        self.m = Figurative(os.path.join(dirname, "binaries", "basic_linux_amd64"), policy="random")
    ...
    def test_state_hooks(self):
        ...
        def process_hook(state: State) -> None:
            ...
            # the Figurative class, not State
...
class StateMergeTest(unittest.TestCase):
    ...
    class StateCounter(Plugin):
        def did_fork_state_callback(self, *_args, **_kwargs):
            self.max_states = max(
                self.max_states,
                self.figurative.count_busy_states()
                + self.figurative.count_ready_states()
                + self.figurative.count_killed_states()
                + self.figurative.count_terminated_states(),
            )

        @property
        def max_states(self):
            with self.figurative.locked_context() as ctx:
