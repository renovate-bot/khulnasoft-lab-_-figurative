...
from figurative.core.smtlib import Solver
from figurative.core.state import Concretize
from figurative.native import Figurative
from figurative.native.cpu.abstractcpu import ConcretizeRegister
from figurative.native.plugins import SyscallCounter

from figurative.platforms import linux, linux_syscall_stubs
from figurative.platforms.linux import SymbolicSocket, logger as linux_logger
from figurative.platforms.platform import SyscallNotImplemented, logger as platform_logger
...
def test_symbolic_syscall_arg() -> None:
    ...
    m = Figurative(BIN_PATH, argv=["+"], workspace_url=str(tmp_dir.name))
...
def test_symbolic_length_recv() -> None:
    ...
    tmp_dir = tempfile.TemporaryDirectory(prefix="mcore_test_")
    m = Figurative(BIN_PATH, workspace_url=str(tmp_dir.name))
...
class LinuxTest(unittest.TestCase):
    ...
    def test_dir_stat(self):
        ...
        # Remove from file system on host but not in Figurative
        os.rmdir(dname)
        self.assertFalse(os.path.exists(dname))
        res = self.linux.sys_stat32(0x1100, 0x1200)
        ...
        # Remove the directory using Figurative
        self.linux.sys_rmdir(0x1100)
        res = self.linux.sys_stat32(0x1100, 0x1200)
        self.assertLess(res, 0)
        res = self.linux.sys_stat64(0x1100, 0x1200)
    ...
    def test_file_stat(self):
        ...
        # Remove the file using Figurative
