...
class RegisterCapturePlugin(Plugin):
    def did_run_callback(self):
        with self.figurative.locked_context("regs", dict) as context:
            for st in self.figurative.terminated_states:
...
class ManticornTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.concrete_instance = Figurative(
            os.path.join(dirname, "binaries", "arguments_linux_amd64"),
            argv=["argv", "mc", "argface"],
        )
...
class UnicornResumeTest(unittest.TestCase):
    ...
    def setUp(self):
        dirname = os.path.dirname(__file__)
        self.concrete_instance = Figurative(os.path.join(dirname, "binaries", "rusticorn"))
