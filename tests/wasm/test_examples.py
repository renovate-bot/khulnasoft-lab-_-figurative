...
from figurative.wasm.cli import wasm_main
...
class TestCollatz(unittest.TestCase):
    def test_getchar(self):
        m = FigurativeWASM(collatz_file, env={"getchar": getchar})
        m.invoke("main")
        m.run()
        results = []
    ...
    def test_symbolic_args(self):
        m = FigurativeWASM(collatz_file, env={})
...
class TestIfCheck(unittest.TestCase):
    def test_getchar(self):
        m = FigurativeWASM(if_check_file, env={"getchar": getchar2})
        m.main()
        results = []
        for idx, val_list in enumerate(m.collect_returns()):
            results.append(val_list[0][0])
